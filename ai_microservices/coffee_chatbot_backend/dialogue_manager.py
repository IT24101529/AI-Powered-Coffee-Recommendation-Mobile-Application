...
async def handle_suggest(message, session_id, session, llm_signal=None, intent='Suggest'):
    ...
    if step == 'ask_mood':
        ...
        if has_health_prefs:
            update_session(session_id, {'step': 'ask_temp'})
            return {
                'reply': f"{tone_prefix}I've noted your preferences! To narrow it down, do you prefer hot or cold drinks today?",
                'quick_replies': ['Hot', 'Cold', 'No preference'],
                'intent': 'Suggest',
                'state': 'GATHERING'
            }
