...
public class KnnApiController {

    @GetMapping("/results")
    public ResponseEntity<String> getResults() {
        try {
            ClassPathResource res = new ClassPathResource("static/knn_results.json");
            ...
            return ResponseEntity.internalServerError().body("{\"error\":\"Run knn_engine.py first.\"}");
        }
    }
    ...
    public ResponseEntity<Map<String,String>> run() {
        try {
            String path = new ClassPathResource("static/knn_engine.py").getFile().getAbsolutePath();
            ProcessBuilder pb = new ProcessBuilder("python3", path);
            pb.redirectErrorStream(true);
            Process p = pb.start();
            String out = new String(p.getInputStream().readAllBytes(), StandardCharsets.UTF_8);
            int exit = p.waitFor();
            if (exit == 0) return ResponseEntity.ok(Map.of("status","success","message","KNN completed."));
